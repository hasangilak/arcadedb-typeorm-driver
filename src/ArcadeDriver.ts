...
export class ArcadeDriver implements Driver {
  ...
  readonly supportedDataTypes: ColumnType[] = [
    'string',
    'integer',
    'long',
    'float',
  ...
  readonly mappedDataTypes: MappedColumnTypes = {
    createDate: 'datetime',
    createDateDefault: 'sysdate()',
    updateDate: 'datetime',
    updateDateDefault: 'sysdate()',
    deleteDate: 'datetime',
  ...
  constructor(
    readonly dataSource: ArcadeDataSource,
    readonly arcadeOptions: ArcadeDataSourceOptions,
  ) {
  ...
  async request(
    ...
    const response = await fetch(
      `${(this.arcadeOptions.url ?? 'http://127.0.0.1:2480').replace(/\/$/, '')}/api/v1/${path}`,
      {
          ...
          body === undefined
            ...
            : JSON.stringify(body, (_key, value) => {
                if (value === undefined || typeof value === 'function' || typeof value === 'symbol')
                  throw new Error('SQL parameters cannot contain undefined, functions or symbols');
                if (typeof value === 'number' && !Number.isFinite(value))
                  throw new Error('SQL parameters must contain finite numbers');
                return value;
              }),
    ...
    try {
      ...
      throw new ArcadeHttpError(
        `ArcadeDB returned a non-JSON response (HTTP ${response.status})`,
        response.status,
      );
    }
    if (!response.ok)
      throw new ArcadeHttpError(
        data.detail ?? data.error ?? `ArcadeDB HTTP ${response.status}`,
        response.status,
        data.exception,
      );
  ...
  async connect(): Promise<void> {
    ...
    if (!body.result) {
      if (!this.arcadeOptions.createDatabase)
        throw new Error(`ArcadeDB database ${this.database} does not exist`);
  ...
  async afterConnect(): Promise<void> {
    for (const metadata of this.dataSource.entityMetadatas) {
      if (metadata.relations.length || metadata.treeType || metadata.tableType !== 'regular') {
        throw new Error(
          `ArcadeDB supports document entities without ORM relations: ${metadata.name}`,
        );
      }
      if (
        metadata.checks.length ||
        metadata.exclusions.length ||
        metadata.indices.some(
          (index) => index.where || index.isFulltext || index.isSpatial || index.type,
        )
      ) {
        throw new Error(
          `Check/exclusion constraints and specialized indexes are not supported by ArcadeDB synchronization: ${metadata.name}`,
        );
      }
      for (const column of metadata.columns) {
        if (column.isGenerated && column.generationStrategy !== 'uuid') {
          throw new Error(
            'ArcadeDB supports generated UUIDs or manually assigned primary keys, not auto-increment',
          );
        }
      }
    }
  ...
  createSchemaBuilder(): ArcadeSchemaBuilder {
    return new ArcadeSchemaBuilder(this);
  }
  ...
  buildTableName(name: string, schema?: string, database?: string): string {
    if (schema || (database && database !== this.database))
      throw new Error('ArcadeDB does not support cross-database tables or SQL schemas');
    return name;
  }
  parseTableName(
    target: Parameters<Driver['parseTableName']>[0],
  ): ReturnType<Driver['parseTableName']> {
    const tableName =
      typeof target === 'string' ? target : 'tableName' in target ? target.tableName : target.name;
  ...
  normalizeType(column: Parameters<Driver['normalizeType']>[0]): string {
    ...
    if (
      type === String ||
      ['text', 'varchar', 'uuid', 'simple-array', 'simple-json'].includes(type as string)
    )
      return 'string';
  ...
  preparePersistentValue(value: any, column: ColumnMetadata): any {
    ...
    if (value === null && !column.isNullable)
      throw new Error(`Column ${column.propertyPath} cannot be null`);
    if (value == null) return value;
    if (column.type === 'simple-json') return JSON.stringify(value);
    if (column.type === 'simple-array') return value.join(',');
    if (value instanceof Date)
      return column.type === 'date' ? value.toISOString().slice(0, 10) : value.toISOString();
    return value;
  }
  ...
  prepareHydratedValue(value: any, column: ColumnMetadata): any {
    if (value != null) {
      ...
      else if (column.type === 'simple-array' && typeof value === 'string')
    ...
    return column.transformer
      ? ApplyValueTransformers.transformFrom(column.transformer, value)
      : value;
  }
  ...
  normalizeIsUnique(column: ColumnMetadata): boolean {
    return column.entityMetadata.uniques.some(
      (unique) => unique.columns.length === 1 && unique.columns[0] === column,
    );
  }
  ...
  async obtainMasterConnection(): Promise<ArcadeDriver> {
    return this;
  }
  ...
  createGeneratedMap(
    ...
    return metadata.columns.reduce((map, column) => {
      if (Object.hasOwn(row, column.databaseName)) {
        OrmUtils.mergeDeep(
          map,
          column.createValueMap(this.prepareHydratedValue(row[column.databaseName], column)),
        );
      }
      return map;
    }, {});
  ...
  findChangedColumns(columns: TableColumn[], metadata: ColumnMetadata[]): ColumnMetadata[] {
    return metadata.filter((column) =>
      columns.some(
        (existing) =>
          existing.name === column.databaseName && existing.type !== this.normalizeType(column),
      ),
    );
  }
}
